...
- **Frontend** (`frontend/`) is a Vite + React 19 dashboard that talks to the backend via `frontend/src/services/api.ts`, reading the JWT from `localStorage`. The API base URL is baked in at build time via `VITE_API_URL` (Docker build arg), not runtime config. `/login` (`pages/LoginPage.tsx`) posts to `/auth/login` and stores the token; `App.tsx`'s `RequireAuth` wrapper redirects unauthenticated visitors there, and `useAuth` (`hooks/useAuth.ts`) reacts to token changes via a `storage`/`auth-change` event pair — the API client dispatches `auth-change` and clears the token on any 401, so an expired session bounces the user back to `/login` without a full page reload.
